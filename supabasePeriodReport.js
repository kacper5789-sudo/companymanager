...
(function () {
  ...
  function setupStyles() {
    ...
    style.textContent = `
      ...
      .cm-period-employee-pill{display:inline-flex;align-items:center;gap:8px;min-height:34px;padding:7px 10px;border:1px solid rgba(148,163,184,.16);border-radius:999px;background:rgba(255,255,255,.055);color:#e5eefb;font-size:12px;font-weight:900;cursor:pointer;user-select:none;transition:background .16s ease,border-color .16s ease,box-shadow .16s ease,transform .16s ease;}
      ...
      html[data-cm-theme]:not([data-cm-theme="original"]) .cm-period-employee-filter{background:transparent!important;background-image:none!important;border:none!important;box-shadow:none!important;}
      html[data-cm-theme]:not([data-cm-theme="original"]) .cm-period-employee-filter-title{color:var(--cm-skin-accent,#2563eb)!important;text-shadow:0 10px 26px color-mix(in srgb,var(--cm-skin-accent,#2563eb) 18%,transparent)!important;}
      html[data-cm-theme]:not([data-cm-theme="original"]) .cm-period-employee-pill{background:linear-gradient(180deg,color-mix(in srgb,var(--cm-skin-accent-2,var(--cm-skin-accent,#2563eb)) 20%,var(--cm-skin-card,#fff) 80%),color-mix(in srgb,var(--cm-skin-accent,var(--cm-skin-accent-2,#2563eb)) 14%,var(--cm-skin-soft,var(--cm-skin-card,#fff)) 86%))!important;border:1px solid color-mix(in srgb,var(--cm-skin-accent,#2563eb) 34%,transparent)!important;color:var(--cm-skin-text,#0f172a)!important;box-shadow:0 10px 24px color-mix(in srgb,var(--cm-skin-shadow,rgba(0,0,0,.16)) 22%,transparent),inset 0 1px 0 color-mix(in srgb,var(--cm-skin-accent-2,var(--cm-skin-accent,#2563eb)) 18%,rgba(255,255,255,.42))!important;}
      html[data-cm-theme]:not([data-cm-theme="original"]) .cm-period-employee-pill:hover{background:linear-gradient(180deg,color-mix(in srgb,var(--cm-skin-accent-2,var(--cm-skin-accent,#2563eb)) 34%,var(--cm-skin-card,#fff) 66%),color-mix(in srgb,var(--cm-skin-accent,var(--cm-skin-accent-2,#2563eb)) 24%,var(--cm-skin-soft,var(--cm-skin-card,#fff)) 76%))!important;border-color:color-mix(in srgb,var(--cm-skin-accent,#2563eb) 54%,transparent)!important;transform:translateY(-1px);}
      html[data-cm-theme]:not([data-cm-theme="original"]) .cm-period-employee-pill input{accent-color:var(--cm-skin-accent,#2563eb)!important;}
  ...
  async function fetchPeriodData(ctx, range) {
    ...
    const [salesRes, paymentsRes, appointmentsRes, clientsRes, employeesRes, employeesTableRes, passesRes, companyRes] = await Promise.all([
  ...
  function buildReport(data, range) {
    ...
    activeSalesForFinance.forEach(s => {
      ...
      const saleHasPass = passBySaleId.has(s.id) || items.some((it) => String(it.sale_id || "") === String(s.id || "") && normalizeItemType(it.item_type) === "pass");
      const row = saleHasPass ? ensureEmployee(null, companySellerName) : ensureEmployee(s.employee_id || appointment.employee_id, s.employee_name, appointment.employee_name);
      row.sales += 1;
    ...
    items.forEach(item => {
      ...
      const type = normalizeItemType(item.item_type);
      const row = type === 'pass'
        ? ensureEmployee(null, companySellerName)
        : ensureEmployee(sale.employee_id || appointment.employee_id, sale.employee_name, appointment.employee_name);
      const qty = itemQty(item);
      const value = reportItemValue(item);
