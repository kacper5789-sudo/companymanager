-- COMPANYMANAGER 049A — company-panel.html Supabase
...
add column if not exists message_sender text,
add column if not exists sms_sender text,
add column if not exists visit_sms_24 boolean default false,
add column if not exists visit_sms_sender text,
add column if not exists visit_sms_template text,
add column if not exists birthday_sms boolean default false,
add column if not exists birthday_sms_sender text,
add column if not exists birthday_sms_template text,
add column if not exists after_add_sms boolean default false,
add column if not exists after_add_sms_sender text,
add column if not exists after_add_sms_template text,
add column if not exists after_visit_sms boolean default false,
add column if not exists after_visit_sms_sender text,
add column if not exists after_visit_sms_template text,
...
add column if not exists working_day_start time default '08:00',
add column if not exists working_day_end time default '18:00',
add column if not exists default_visit_duration_minutes integer default 30;

create or replace function public.company_panel_get()
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if me.role <> 'OWNER' and me.company_id <> c.id then
...
grant execute on function public.company_panel_get() to authenticated;
