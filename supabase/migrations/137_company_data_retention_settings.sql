-- COMPANYMANAGER 049F
-- Panel firmy: czas przechowywania danych + porządek ustawień programu
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'companies_data_retention_months_allowed'
  ) then
    alter table public.companies
      add constraint companies_data_retention_months_allowed
      check (data_retention_months is null or data_retention_months in (6,12,24,36,48,60));
  end if;
end $$;
...
begin
  ...
  set
    ...
    data_retention_months = case
      when p_payload ? 'data_retention_months' and (p_payload->>'data_retention_months') is null then null
      when p_payload ? 'data_retention_months' and nullif(p_payload->>'data_retention_months', '') is null then null
      when p_payload ? 'data_retention_months' and (p_payload->>'data_retention_months')::integer in (6,12,24,36,48,60) then (p_payload->>'data_retention_months')::integer
      else data_retention_months
    end
