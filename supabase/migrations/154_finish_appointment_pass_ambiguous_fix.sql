-- 154_finish_appointment_pass_ambiguous_fix.sql
-- CompanyManager: naprawa błędu "column reference pass_used_value is ambiguous" przy kończeniu wizyty.
-- Przyczyna: zmienne PL/pgSQL miały tę samą nazwę co kolumny appointments.pass_used_value/pass_used_units.
-- Fix: funkcja używa zmiennych v_pass_used_value / v_pass_used_units i nie koliduje z kolumnami.
...
begin
  ...
  if effective_pass_id is not null then
    ...
    if v_pass.service_id is not null and a.service_id is not null and v_pass.service_id <> a.service_id then
  ...
  if a.service_id is not null then
    ...
    values (final_company_id, v_sale_id, 'service', a.service_id, effective_pass_id, coalesce(service_item_name, a.service_name, 'Usługa'), coalesce(service_item_name, a.service_name, 'Usługa'), 1, service_unit_price, v_pass_used_value, service_charge, service_charge, now());
  end if;
