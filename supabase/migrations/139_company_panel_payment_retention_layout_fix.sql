-- COMPANYMANAGER 049H
-- Panel firmy: czas przechowywania danych + poprawione ustawienia metod płatności.
...
begin
  ...
  set
    ...
    data_retention_months = case
      when not (p_payload ? 'data_retention_months') then data_retention_months
      when nullif(p_payload->>'data_retention_months', '') is null then null
      when (p_payload->>'data_retention_months')::integer in (6,12,24,36,48,60) then (p_payload->>'data_retention_months')::integer
