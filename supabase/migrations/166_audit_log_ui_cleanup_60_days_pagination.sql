-- COMPANYMANAGER — 080 COMPANY AUDIT LOG UI CLEANUP + 60 DAYS + PAGINATION
-- Dziennik aktywności per firma.
-- Domyślnie aplikacja pokazuje ostatnie 60 dni, a baza czyści starsze wpisy.
-- OWNER może wejść w kontekst firmy i oglądać jej dziennik, bez mieszania firm.
...
create index if not exists idx_company_audit_logs_company_created_id on public.company_audit_logs(company_id, created_at desc, id desc);

create or replace function public.cm_cleanup_company_audit_logs_60_days()
returns trigger
language plpgsql
security definer
...
as $$
begin
  delete from public.company_audit_logs
...
drop trigger if exists trg_cm_cleanup_company_audit_logs_60_days on public.company_audit_logs;
create trigger trg_cm_cleanup_company_audit_logs_60_days
after insert on public.company_audit_logs
for each row execute function public.cm_cleanup_company_audit_logs_60_days();
...
drop function if exists public.cm_list_company_audit_logs(uuid, integer, text, text, text, timestamptz, timestamptz);
drop function if exists public.cm_list_company_audit_logs(uuid, integer, integer, text, text, text, timestamptz, timestamptz);
...
begin
  ...
  final_limit := least(greatest(coalesce(p_limit, 100), 1), 200);

  if upper(coalesce(me.role::text,'')) = 'OWNER' and final_company_id is null then
  ...
  where (final_company_id is null or l.company_id = final_company_id)
    ...
    and l.created_at >= greatest(coalesce(p_date_from, now() - interval '60 days'), now() - interval '60 days')
...
grant execute on function public.cm_list_company_audit_logs(uuid, integer, integer, text, text, text, timestamptz, timestamptz) to authenticated;
