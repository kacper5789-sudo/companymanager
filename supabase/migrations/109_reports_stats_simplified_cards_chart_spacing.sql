-- COMPANYMANAGER 109
-- Wykres/Statystyka: uproszczone kafle, przychód dziś i estetyczne wykresy.
...
begin
  ...
  with periods as (
    ...
    where s.company_id = p_company_id
      ...
      and coalesce(lower(s.payment_status::text), '') not in ('void', 'unpaid', 'pending', 'nieopłacone', 'nieoplacone')
  ), today_revenue_stats as (
    select
      coalesce(sum(coalesce(si.total, si.total_price, si.unit_price * coalesce(si.quantity, 1), tsb.total_gross, tsb.total_net, 0)), 0)::numeric(12,2) as today_revenue,
      count(distinct tsb.id)::int as today_sales_count
    from today_sales_base tsb
    left join public.sale_items si on si.sale_id = tsb.id
  ), item_stats as (
    select
      sb.period_start,
  ...
  select jsonb_build_object(
    ...
    'summary', jsonb_build_object(
      ...
      'today_revenue', coalesce((select today_revenue from today_revenue_stats), 0),
      'today_sales_count', coalesce((select today_sales_count from today_revenue_stats), 0),
